...
func cleanText(s string) string {
	stripped := tagRE.ReplaceAllString(scrubText(s), " ")
...
func (c *Client) Categories() ([]domain.Category, error) {
...
func parseCategories(html string) []domain.Category {
	...
	for _, m := range categoryAnchorRE.FindAllStringSubmatch(html, -1) {
		...
		seen[path] = true
		out = append(out, domain.Category{Name: name, Path: path})
	}
	return out
}
...
func (c *Client) Subcategories(path string) ([]domain.Category, error) {
...
func parseSubcategories(html, parent string) []domain.Category {
	var out []domain.Category
	seen := make(map[string]bool)
	for _, m := range subcatAnchorRE.FindAllStringSubmatch(html, -1) {
		...
		seen[p] = true
		out = append(out, domain.Category{Name: name, Path: p})
	}
	return out
}
