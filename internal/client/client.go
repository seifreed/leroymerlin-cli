...
type Client struct {
	...
	// Cookie carries the browser session — the DataDome clearance among the rest —
	// that every read runs on. DataDome challenges a client it does not recognise,
	// so an empty Cookie means a 403 from the storefront, not an anonymous read.
	Cookie   string
	cookieMu sync.RWMutex

	// TabID identifies this checkout client session to the storefront API.
	TabID string
...
// New returns a Client with web-app-like defaults (no session yet, Spanish). Its HTTP
...
// acceptLanguage is what a Spanish Chrome sends. The storefront serves Spanish
// only and ignores this header — a request made with a Catalan one comes back
// byte for byte the same — so there is nothing to choose between.
const acceptLanguage = "es-ES,es;q=0.9,en;q=0.8"
...
func (c *Client) do(req *http.Request) ([]byte, error) {
	...
	// Only the storefront may write to the session. resp.Request.URL is the URL
	// after redirects, so a page that bounces the client to a foreign host cannot
	// plant a cookie that later rides along on a request to leroymerlin.es.
	if resp.Request != nil && c.sameOrigin(resp.Request.URL.String()) {
		for _, cookie := range resp.Cookies() {
			c.applyResponseCookie(cookie)
		}
	}
...
func (c *Client) resolve(u string) string {
	...
	return strings.TrimRight(c.BaseURL, "/") + u
}
