...
Spanish is the default language (`--lang ca` for Catalan). Reads need no login.

## Authenticate (bring-your-own session)

Search / price / categories need **no login**. The cart needs a browser cookie. Leroy Merlin fronts the
site with **DataDome** bot protection: anonymous *reads* clear it via the CLI's Chrome TLS fingerprint,
but the *cart* endpoints are stricter (they return HTTP 403 even to the CLI without a cookie). There is
no account login to automate — the cookie is just the DataDome clearance + session. The user opens
`leroymerlin.es` once in their **own browser** (residential IP), then hand the session to the CLI:

1. **`leroymerlin login --from-browser chrome`** — the easy path, prefer it. Reads the cookie straight
   from the browser's cookie store (`chrome`|`chromium`|`firefox`|`safari`|`edge`|`brave`; empty = any
   installed). No DevTools, no files. On macOS Chrome it pops a one-time Keychain prompt. Self-verifies
   and prints `ok — cookie lifted from browser, reads working`.
...
`leroymerlin login --from-browser chrome` to lift a fresh cookie.
